...
### 新增
- **DID 位级（Bit Fields）全量解码**：调查表 4_1 的 Bit 段范围（0~3）与单位段（Bit 7）均解析为
  位段子字段，4_2 表 IO DID 的 OFF/ON 枚举按位合并；0x0600 学习状态 0x11 显示
  "前轮传感器ID: 已学习；后轮传感器ID: 已学习"，3200 灯光控制逐位显示 ON/OFF。
- **会话保持功能寻址与抑制正响应配置**：保活报文可选物理/功能寻址（DoCAN 功能 CAN ID /
  DoIP 功能组逻辑地址 0xE400，0x7DF 自动映射），抑制正响应默认开启（3E 80）；
  连接面板按硬件类型适配功能寻址 ID 配置，随 ECU 定义同步并可临时覆盖。
- **定义库统一入口**（definition_center/definition_picker）：DID/DTC/IO/例程面板下拉
  直达定义库；可编辑下拉双击弹候选（输入与选单两不误）。
- **CANoe 风格诊断控制台**：结构化请求编辑 + 预览 + 响应解析，原始 HEX 模式移除。

### 修复
- DoIP 模式显示全面修正：通信信息协议字段随传输类型切换（DoIP (ISO 13400) + UDS）；
  Trace 分层显示——DoIP Trace 显示完整 DoIP 帧（02 FD 80 01 + 源/目标逻辑地址），
  UDS Trace 保持纯 UDS 会话层数据；页签/表头随 DoIP 会话切换为逻辑地址语义。
- 调查表解析：范围列 hex 写法兼容（0x0F）、单位占位符归一化、删除线条目剔除。
...
### 新增
- **用例前置条件/清理步骤**（对应 Diva TestModule 的 setup/teardown）:
  - pre_steps 前置步骤: 用例前执行，任一失败则判失败并跳过主步骤（如前置扩展会话失败）
  - post_steps 清理步骤: 用例后总执行（会话还原等），失败仅警告不影响判定；
    即使主步骤失败/中止，清理仍保证执行，避免ECU残留非默认会话
  - JSON 字段 pre_steps/post_steps，示例序列已演示完整结构
- **测试报告含每步报文明细**: 导出CSV改为步骤级明细——每行一步，
  含阶段（前置/主步骤/清理）、序号、名称、请求报文TX、响应报文RX、
  步骤结果、耗时、错误信息（不再仅步骤数摘要）；未运行时退化为摘要导出
...
- **用例库会话依赖重构**: SA-02/SA-03/DT-04/DT-05/DD-04 改为前置扩展会话（10 03）+
  清理还原默认会话（10 01），修复实车上 85/2E 等服务因默认会话回 NRC 0x7F 的用例失败
